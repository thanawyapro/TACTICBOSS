...
const buildShareCard = async (item: CommunityTactic, url: string): Promise<File> => {
  ...
  ctx.fillStyle = '#c4b5fd'; ctx.font = '700 38px sans-serif'; ctx.fillText('TACTIC BOSS AI • COMMUNITY', 110, 150);
...
export default function CommunityLite({ userId, coachName, lang, savedTactics, onUseTactic, onToast }: Props) {
  ...
  const load = async (force = false) => {
    ...
    if (error) onToast(text(lang, 'تعذر تحميل المجتمع. شغّل Migration v58 أولًا.', 'Community could not load. Apply the v58 migration first.'));
