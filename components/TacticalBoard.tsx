...
import { Eraser, MousePointerClick, Pencil, RotateCcw, Save, UserRoundCog, Layers, ShieldCheck, Route, Wand2, BarChart3 } from 'lucide-react';
...
import { analyzeBoardShape } from '../utils/boardIntelligence';
...
const TacticalBoard = ({ formation, opponentFormation, lang, value, onChange, readOnly = false, showOpponent = true, compact = false }: TacticalBoardProps) => {
  const copy = ui[lang];
  ...
  // V131.6 improvement: Board insights from analyzeBoardShape
  const boardInsights = useMemo(() => {
    if (compact || readOnly) return [];
    return analyzeBoardShape(board, lang as SupportedLang);
  }, [board, lang, compact, readOnly]);
  ...
  // V131.6 Deep Improvement: Smart suggestions based on board analysis
  const smartSuggestions = useMemo(() => {
    if (!boardInsights.length) return [];
    const suggestions: string[] = [];
    if (boardInsights.some(i => i.includes('ظهيران') || i.includes('fullbacks'))) {
      suggestions.push(lang === 'ar' ? 'اضغط على زر "حماية العمق" لإصلاح الثغرة الدفاعية' : 'Use "Protect depth" button to fix defensive gap');
    }
    if (boardInsights.some(i => i.includes('ضغط زائد') || i.includes('pushed high'))) {
      suggestions.push(lang === 'ar' ? 'استخدم "ضغط الشكل" لإعادة التوازن' : 'Use "Compact shape" to restore balance');
    }
    return suggestions;
  }, [boardInsights, lang]);
    ...
    <div className="space-y-3">
      {!readOnly && (
        <div className="glass-card p-3 rounded-xl space-y-3 text-[10px]">
          ...
          {/* V131.6: Board Insights Panel */}
          {showInsights && boardInsights.length > 0 && (
            <div className="mt-2 rounded-xl border border-emerald-400/20 bg-emerald-950/20 p-3 text-[10px]">
              <div className="flex items-center gap-2 mb-2 text-emerald-300 font-bold">
                <BarChart3 size={14} /> {lang === 'ar' ? 'تحليل السبورة الذكي' : 'Smart Board Analysis'}
              </div>
          ...
          <div className="flex gap-1.5">
            ...
            {!compact && !readOnly && (
              <button type="button" onClick={() => setShowInsights(!showInsights)} className={`px-3 py-2 rounded-lg flex gap-1.5 items-center text-xs ${showInsights ? 'bg-emerald-600 text-white' : 'bg-white/5 text-slate-300'}`}>
                <BarChart3 size={13} /> {lang === 'ar' ? 'تحليل السبورة' : 'Analyze Board'}
              </button>
            )}
          </div>
          ...
          {/* V131.6 Deep: Smart Suggestions from Analysis */}
          {smartSuggestions.length > 0 && (
            <div className="mt-2 p-2 rounded-xl bg-emerald-950/30 border border-emerald-400/20 text-[9px] text-emerald-200">
              <div className="font-bold mb-1">اقتراحات ذكية:</div>
              {smartSuggestions.map((s, i) => <div key={i}>• {s}</div>)}
            </div>
          )}
        </div>
      )}
