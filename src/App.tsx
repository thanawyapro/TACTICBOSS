...
export default function App() {
  ...
  const createProShareCardBlob = async () => {
    ...
    try {
      ...
      logo.src = "/assets/brand-logo-wide.png?v=105";
  ...
  const aiTextGuard = canUseLocalAi(normalizedCompetition, subscription.plan, "text_generation");
  const aiTextLeft = Math.max(0, aiTextGuard.limit - aiTextGuard.used);
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 max-w-md mx-auto px-4 pt-4 sm:max-w-lg md:max-w-xl">
        ...
        {/* ======================================================= */}
        {screen === "home" && (
          <div className="space-y-4 pt-3 pb-6 animate-fade-in">
            {/* V105 Calm Game Hub: one navigation system only, no duplicated top tabs */}
            <div className="rounded-[2rem] border border-cyan-400/10 bg-slate-950/45 px-4 py-3 shadow-2xl shadow-cyan-950/10 backdrop-blur-xl">
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-3 min-w-0">
                  <img
                    src="/assets/brand-logo-wide.png?v=105"
                    alt="Tactic Boss AI"
                    className="h-11 max-w-[168px] object-contain drop-shadow-[0_0_14px_rgba(34,211,238,0.22)]"
                  />
                </div>
                <button
                  type="button"
                  onClick={() => setScreen("subs")}
                  className="shrink-0 rounded-full border border-amber-300/25 bg-amber-400/10 px-3 py-1.5 text-[10px] font-black text-amber-200"
                >
                  {subscription.plan === "free"
                    ? uiText("Free", "Free", "Free", "Free")
                    : subscription.plan.toUpperCase()}
                </button>
              </div>

              <div className="mt-3 grid grid-cols-4 gap-2 text-center">
                <button onClick={() => setScreen("progression")} className="rounded-2xl border border-white/5 bg-white/[0.035] px-2 py-2">
                  <div className="text-[9px] text-slate-500">{uiText("المستوى", "Level", "Nivel", "Niveau")}</div>
                  <div className="mt-0.5 text-xs font-black text-white">Lv {tacticalLevel.level}</div>
                </button>
                <button onClick={() => setScreen("progression")} className="rounded-2xl border border-white/5 bg-white/[0.035] px-2 py-2">
                  <div className="text-[9px] text-slate-500">{uiText("السلسلة", "Streak", "Racha", "Série")}</div>
                  <div className="mt-0.5 text-xs font-black text-white">🔥 {normalizedCompetition.currentStreak || progression.streak || 0}</div>
                </button>
                <button onClick={() => setScreen("progression")} className="rounded-2xl border border-cyan-300/10 bg-cyan-400/[0.035] px-2 py-2">
                  <div className="text-[9px] text-slate-500">AI</div>
                  <div className="mt-0.5 text-xs font-black text-cyan-100">{aiTextLeft}/{aiTextGuard.limit}</div>
                </button>
                <button onClick={() => setScreen("progression")} className="rounded-2xl border border-violet-300/10 bg-violet-400/[0.035] px-2 py-2">
                  <div className="text-[9px] text-slate-500">XP</div>
                  <div className="mt-0.5 text-xs font-black text-violet-100">{progression.xp}</div>
                </button>
              </div>
            </div>

            <section className="relative overflow-hidden rounded-[2rem] border border-cyan-400/25 bg-[radial-gradient(circle_at_70%_20%,rgba(34,211,238,0.22),transparent_35%),linear-gradient(135deg,rgba(15,23,42,0.95),rgba(8,13,24,0.96))] p-5 shadow-2xl shadow-cyan-950/20">
              <div className="absolute inset-y-0 left-0 w-1/2 bg-[radial-gradient(circle_at_center,rgba(168,85,247,0.16),transparent_58%)] pointer-events-none" />
              <div className="relative z-10 grid grid-cols-[1fr_auto] gap-4 items-center">
                <div className="text-right">
                  <div className="inline-flex items-center gap-1.5 rounded-full border border-amber-300/20 bg-amber-300/10 px-2.5 py-1 text-[10px] font-black text-amber-200">
                    <CalendarDays size={12} />
                    {uiText("تحدي اليوم", "Daily Challenge", "Reto diario", "Défi du jour")}
                  </div>
                  <h1 className="mt-3 text-2xl font-black leading-tight text-white">
                    {uiText("أوقف", "Stop", "Detén", "Stoppez")} {dailyRivalChallenge.formation}
                  </h1>
                  <p className="mt-1 text-sm font-bold text-slate-300">
                    {dailyRivalChallenge.style} • <span className="text-amber-200">+{dailyRivalChallenge.reward} XP</span>
                  </p>
                  <div className="mt-4 flex items-center gap-2 justify-end">
                    <button
                      type="button"
                      onClick={() => {
                        setSelectedGame(GAMES_LIST.find((g) => g.id === homeGameId) || GAMES_LIST[0]);
                        setGeneratorMode("counter");
                        setStep(1);
                        setScreen("generator");
                      }}
                    ...
                    >
                      {uiText("ابدأ الآن", "Start now", "Empezar", "Commencer")}
                    </button>
                    <button
                      type="button"
                      onClick={() => setScreen("competition")}
                      className="rounded-2xl border border-white/10 bg-white/[0.04] px-3 py-3 text-xs font-bold text-slate-200"
                    >
                      {uiText("الدوري", "League", "Liga", "Ligue")}
                    </button>
                  </div>
                </div>

                <div className="hidden min-[420px]:flex h-28 w-28 shrink-0 items-center justify-center rounded-3xl border border-cyan-300/15 bg-cyan-400/8">
                  <div className="relative h-20 w-20 rounded-2xl border border-cyan-300/30 bg-slate-950/70">
                    <span className="absolute left-3 top-3 h-2.5 w-2.5 rounded-full bg-cyan-300 shadow-[0_0_16px_rgba(34,211,238,0.9)]" />
                    <span className="absolute right-4 top-8 h-2.5 w-2.5 rounded-full bg-violet-300 shadow-[0_0_16px_rgba(168,85,247,0.9)]" />
                    <span className="absolute bottom-4 left-8 h-2.5 w-2.5 rounded-full bg-cyan-300 shadow-[0_0_16px_rgba(34,211,238,0.9)]" />
                    <span className="absolute inset-x-4 top-1/2 border-t border-dashed border-cyan-300/35" />
                    <span className="absolute inset-y-4 left-1/2 border-l border-dashed border-violet-300/35" />
                  </div>
                </div>
              </div>
            </section>
            ...
            <div className="grid grid-cols-2 gap-3">
              <button onClick={startBuildMyTactic} className="group rounded-3xl border border-cyan-300/20 bg-white/[0.04] p-4 text-right hover:bg-white/[0.07] transition">
                <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-2xl border border-cyan-300/20 bg-cyan-400/10">
                  <Sparkles size={23} className="text-cyan-100" />
                </div>
                <h3 className="text-base font-black text-white">{uiText("ابنِ خطتي", "Build My Tactic", "Crear mi táctica", "Créer ma tactique")}</h3>
                <p className="mt-1 min-h-9 text-[11px] leading-relaxed text-slate-400">{uiText("أفضل خطة لأسلوبك.", "Best tactic for your style.", "La mejor táctica.", "Meilleure tactique.")}</p>
                <div className="mt-3 inline-flex rounded-full bg-cyan-400/10 px-2.5 py-1 text-[10px] font-black text-cyan-200">+20 XP</div>
              </button>

              <button onClick={startCounterOpponent} className="group rounded-3xl border border-violet-300/20 bg-white/[0.04] p-4 text-right hover:bg-white/[0.07] transition">
                <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-2xl border border-violet-300/20 bg-violet-400/10">
                  <Sword size={23} className="text-violet-100" />
                </div>
                <h3 className="text-base font-black text-white">{uiText("اضرب خصمي", "Counter My Rival", "Contrarrestar rival", "Contrer l’adversaire")}</h3>
                <p className="mt-1 min-h-9 text-[11px] leading-relaxed text-slate-400">{uiText("خطة مضادة واضحة.", "A clear counter plan.", "Plan de contra.", "Plan de contre.")}</p>
                <div className="mt-3 inline-flex rounded-full bg-violet-400/10 px-2.5 py-1 text-[10px] font-black text-violet-200">+40 XP</div>
              </button>

              <button onClick={startAnalyzeMyMatch} className="group rounded-3xl border border-emerald-300/20 bg-white/[0.04] p-4 text-right hover:bg-white/[0.07] transition">
                <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-2xl border border-emerald-300/20 bg-emerald-400/10">
                  <BarChart3 size={23} className="text-emerald-100" />
                </div>
                <h3 className="text-base font-black text-white">{uiText("حلّل المباراة", "Analyze Match", "Analizar partido", "Analyser")}</h3>
                <p className="mt-1 min-h-9 text-[11px] leading-relaxed text-slate-400">{uiText("صورة أو أرقام لقرار.", "Stats or image to decision.", "Datos a decisión.", "Stats en décision.")}</p>
                <div className="mt-3 inline-flex rounded-full bg-emerald-400/10 px-2.5 py-1 text-[10px] font-black text-emerald-200">+50 XP</div>
              </button>

              <button onClick={startLiveRescue} className="group rounded-3xl border border-amber-300/20 bg-white/[0.04] p-4 text-right hover:bg-white/[0.07] transition">
                <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-2xl border border-amber-300/20 bg-amber-400/10">
                  <Zap size={23} className="text-amber-100" />
                </div>
                <h3 className="text-base font-black text-white">{uiText("أنقذني", "Rescue Me", "Rescate", "Sauvetage")}</h3>
                <p className="mt-1 min-h-9 text-[11px] leading-relaxed text-slate-400">{uiText("قرار سريع أثناء اللعب.", "Fast in-match decision.", "Decisión rápida.", "Décision rapide.")}</p>
                <div className="mt-3 inline-flex rounded-full bg-amber-400/10 px-2.5 py-1 text-[10px] font-black text-amber-200">+30 XP</div>
              </button>
            </div>

            <div className="grid grid-cols-2 gap-3">
              <button onClick={() => setScreen("progression")} className="rounded-3xl border border-white/7 bg-white/[0.035] p-4 text-right hover:bg-white/[0.06] transition">
                <div className="flex items-center justify-between gap-2">
                  <div>
                    <div className="text-[11px] font-black text-white">{uiText("مركز اليوم", "Today Center", "Centro", "Centre")}</div>
                    <div className="mt-1 text-[10px] text-slate-400">{uiText("تقدمك اليومي", "Daily progress", "Progreso", "Progression")}</div>
                  </div>
                  <Star size={20} className="text-amber-200" />
                </div>
                <div className="mt-3 text-lg font-black text-amber-200">#{Math.max(1, 128 - Math.floor(normalizedCompetition.weeklyPoints / 25))}</div>
              </button>

              <button onClick={() => setScreen("competition")} className="rounded-3xl border border-fuchsia-300/15 bg-white/[0.035] p-4 text-right hover:bg-white/[0.06] transition">
                <div className="flex items-center justify-between gap-2">
                  <div>
                    <div className="text-[11px] font-black text-white">{uiText("دوري المدربين", "Coach League", "Liga", "Ligue")}</div>
                    <div className="mt-1 text-[10px] text-slate-400">{competitionRankTitle}</div>
                  </div>
                  <Trophy size={20} className="text-fuchsia-200" />
                </div>
                <div className="mt-3 text-lg font-black text-fuchsia-200">{normalizedCompetition.weeklyPoints}</div>
              </button>
            </div>
                ...
                <div className="text-right">
                  <div className="text-[11px] font-black text-slate-200">
                    {nextProgressionUnlock
                      ? uiText("الميزة القادمة", "Next unlock", "Próximo desbloqueo", "Prochain déblocage")
                      : uiText("كل المميزات مفتوحة", "All features unlocked", "Todo desbloqueado", "Tout est débloqué")}
                  </div>
                  <div className="mt-1 text-sm font-black text-cyan-200">
                    {nextProgressionUnlock ? nextProgressionUnlock.title[lang] : uiText("مدرب Elite", "Elite Coach", "Coach élite", "Coach Elite")}
                  </div>
                  <div className="mt-1 text-[10px] text-slate-500">
                    {nextProgressionUnlock
                      ? uiText(`عند Level ${nextProgressionUnlock.level}`, `At Level ${nextProgressionUnlock.level}`, `Nivel ${nextProgressionUnlock.level}`, `Niveau ${nextProgressionUnlock.level}`)
                      : uiText("استمر في المنافسة", "Keep competing", "Sigue compitiendo", "Continuez")}
                  </div>
                </div>
                <button
                  onClick={() => setScreen("progression")}
                  className="shrink-0 rounded-2xl border border-cyan-300/15 bg-cyan-400/10 px-3 py-2 text-[10px] font-black text-cyan-100"
                >
                  {uiText("عرض", "View", "Ver", "Voir")}
                </button>
              </div>
              <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-white/5">
                <div
                  className="h-full rounded-full bg-gradient-to-r from-cyan-400 to-violet-400"
                  style={{ width: `${Math.max(5, Math.min(100, tacticalLevel.progress))}%` }}
                />
              </div>
            </div>

            <div className="grid grid-cols-3 gap-2">
              <button onClick={() => setScreen("meta-center")} className="rounded-2xl bg-white/[0.03] border border-white/5 p-3 text-center hover:bg-white/[0.06]">
                <Target size={17} className="mx-auto text-rose-300" />
                <div className="mt-2 text-[10px] text-slate-200 font-bold">{uiText("الميتا", "Meta", "Meta", "Méta")}</div>
              </button>
              <button onClick={() => setScreen("community")} className="rounded-2xl bg-white/[0.03] border border-white/5 p-3 text-center hover:bg-white/[0.06]">
              ...
              <button onClick={() => { resetToolWorkspace(); setScreen("sandbox-board"); }} className="rounded-2xl bg-white/[0.03] border border-white/5 p-3 text-center hover:bg-white/[0.06]">
      ...
      <div className="fixed bottom-0 left-0 right-0 h-16 bg-slate-950/90 backdrop-blur-md border-t border-white/5 flex items-center justify-around z-40 select-none max-w-md mx-auto sm:max-w-lg md:max-w-xl rounded-t-2xl shadow-xl">
        ...
        <button
          onClick={() => setScreen("competition")}
          className={`flex flex-col items-center justify-center gap-0.5 text-[10px] font-bold h-full w-14 transition ${screen === "competition" ? "text-violet-400 scale-105" : "text-slate-400 hover:text-white"}`}
          id="tab-competition"
        >
          <Trophy size={18} />
          <span>{uiText("المنافسة", "League", "Liga", "Ligue")}</span>
        </button>
