...
export const analyzeBoardShape = (board: TacticalBoardState | null, activeLang: SupportedLang): string[] => {
  ...
  const advancedFullbacksCount = fullbacks.filter(p => p.y < 64).length;
  if (advancedFullbacksCount >= 2) insights.push(m.fullbacks);
  ...
  // New rule: Advanced fullbacks without proper holding protection (enhanced)
  const advancedFullbacks = fullbacks.filter(p => p.y < 55);
  if (advancedFullbacks.length >= 2 && anchors.length === 0) {
    insights.push(activeLang === 'ar' ? 'الظهيران متقدمان بدون لاعب ارتكاز؛ الفريق معرض للمرتدات من الأطراف.' : 'Both fullbacks advanced without a holding midfielder; vulnerable to wide counters.');
  }

  // New rule: Too many players pushed high without defensive cover
  const highPlayers = own.filter(p => p.y < 45);
  if (highPlayers.length >= 5 && anchors.length === 0) {
    insights.push(activeLang === 'ar' ? 'أكتر من 5 لاعبين في الثلث الهجومي بدون تغطية دفاعية كافية.' : 'More than 5 players pushed high with insufficient defensive cover.');
  }
