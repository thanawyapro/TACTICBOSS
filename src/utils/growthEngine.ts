...
export type ActivityType = 'generate' | 'save' | 'rival' | 'improve' | 'daily_plan' | 'challenge' | 'match_analysis' | 'screenshot_analysis' | 'reward_ad' | 'redeem_generation_credit' | 'analyze_board' | 'use_meta' | 'use_tight_marking' | 'board_insight_view';
...
export const xpReward: Record<ActivityType, number> = {
  ...
  analyze_board: 20,
  use_meta: 30,
  use_tight_marking: 15,
  board_insight_view: 10
};
...
export function dailyChallenges(lang: SupportedLang): DailyChallenge[] {
  ...
  const pool: DailyChallenge[] = [
    { id: `daily-counter-${d}`, title: text('تحدي اليوم: اضرب خصمًا افتراضيًا','Daily challenge: counter a simulated rival','Reto diario: contrarresta rival','Défi du jour : contrer un rival'), description: text('افتح اضرب خصمك واختر مشكلة اليوم لتحصل على Rescue Card قابل للمشاركة.', 'Open Counter Plan and solve today’s rival problem with a shareable Rescue Card.', 'Abre Counter Rescue.', 'Ouvrez Counter Rescue.'), xp: 60, tier: 'hard', action: 'rival' },
    ...
    { id: `build-own-${d}`, title: text('ابنِ خطتك بالمحرك التكتيكي','Build your tactic','Crea tu táctica','Créez votre tactique'), description: text('استخدم مسار Build My Tactic لاختيار أفضل/آمن/كرييتڤ.', 'Use Build My Tactic to choose Best/Safe/Creative.', 'Usa Build My Tactic.', 'Utilisez Build My Tactic.'), xp: 35, tier: 'easy', action: 'generate' },
    { id: `board-fix-${d}`, title: text('افحص السبورة بالمحرك التكتيكي','Tactical board scan','Análisis de pizarra','Analyse du tableau'), description: text('راجع قراءة السبورة ونقطة الضعف قبل حفظ الخطة.', 'Review the board scan and weak point before saving.', 'Revisa pizarra.', 'Vérifiez le tableau.'), xp: 35, tier: 'medium', action: 'improve' },
